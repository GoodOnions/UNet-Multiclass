...
class FrameGenerator():
    ...
    def convert_photo_to_binary(self, photo):

        # Convert the photo to grayscale
        grayscale_photo = cv2.cvtColor(photo, cv2.COLOR_BGR2GRAY)

        # Apply thresholding to binarize the photo
        binary_photo = np.where(grayscale_photo != 0, 255, 0)

        # Save the binary photo
        return binary_photo

    def GenerateSamples(self, init_filenumber, end_filenumber, seed, w_tqdm=False, dir='dataset'):
        ...
        for i in files:
            ...
            cv2.imwrite(dir+"/imgs/%d.png" % (i), frame)
            sframe = cv2.cvtColor(sframe.astype(np.uint8), cv2.COLOR_RGB2BGR)

            if False:
                sframe = self.convert_photo_to_binary(sframe)

            cv2.imwrite(dir+"/masks/%d.png" % (i), sframe)
            cv2.imwrite(dir+"/lables/%d.png" % (i), classframe)

        # image_list.write(str(filename))

    def GenerateDataset(self, samples, dir='dataset'):
        ...
        os.makedirs(dir + '/lables/')

        # If only uses one core, execute the function once
        if self.cores == 1:
            level = np.random.choice([0, 1])

            self.GenerateSamples(0, samples, level, w_tqdm=True, dir=dir)
        else:
            ...
            ppool.starmap(self.GenerateSamples, zip(
                ranges[:-1], ranges[1:], seeds, itertools.repeat(False),itertools.repeat(dir)))
